...
- `infrastructure/`: implements those protocols (HTTP quote providers, Postgres). It also loads the
  YAML config into application types and reads settings.
- `api/`: FastAPI. It wires dependencies with `Depends` and translates HTTP, nothing else.

Dependencies point inwards: `api → application → domain`, and `infrastructure → application,
domain`. Never the other way. There are no module-level instances or singletons: the DB engine
and the HTTP client live in the FastAPI `lifespan` and are injected.

## Money

- Use `Decimal` everywhere, never `float`. JSON is parsed with `parse_float=Decimal`, and the API
  sends amounts as decimal strings.
- All rounding lives in `domain/money.py`. Step outputs round **down** to the minor unit and fees
  round **up**. Nothing else rounds.
- Intermediate arithmetic goes through the `Money` operators or `money.mul`/`money.div`, which use
  a fixed decimal context. Never multiply bare `Decimal`s in the domain.
...
- Users can edit every fee. The application layer validates overrides before they reach the
  ...
  `application/limits.py`. Every route is always computed.
...
- Use one branch and one PR per part (`feat/…`, `fix/…`, `refactor/…`, `chore/…`), branched from
  `main`.
